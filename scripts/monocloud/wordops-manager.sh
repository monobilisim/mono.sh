...
backup_site() {
    ...
    for site in "${site_list[@]}"; do
        ...
        echo "$color_blue[ INFO ] Site type for \"$site\" is \"${site_type:-unknown}\"..."
        local site_path=/var/www/$site
        [[ ! -d $site_path ]] && { echo "$color_red[ FAIL ] Site \"$site\" not found at $site_path"; return 1; }
        ...
        if [[ "$site_type" == "wp" ]]; then
            ...
            [[ ! -e $wp_config_file ]] && { echo "$color_red[ FAIL ] wp-config.php not found for \"$site\""; return 1; }

            echo "$color_blue[ INFO ] Taking backup of existing site for \"$site\"..."
            wp --allow-root --path=/var/www/$site/htdocs db export $site.sql &>/dev/null && echo "$color_green[  OK  ] Database backup exported successfully in $site_path/htdocs/$site.sql..." || { echo "$color_red[ FAIL ] Failed to export database backup..."; return 1; }
	    ...
	    cd ..
            ...
            mysqldump $db_name -h localhost -u $db_user -p$db_pass >$site.sql && echo "$color_green[  OK  ] Database backup exported successfully..." || { echo "$color_red[ FAIL ] Failed to export database backup..."; return 1; }
            echo "$color_green[  OK  ] Backup of existing site taken successfully..."
            echo "$color_blue[ INFO ] Writing collected site information to siteinfo.txt..."
            echo "wo_site_name=$site" >siteinfo.txt
            echo "wo_site_type=$site_type" >>siteinfo.txt
            ...
            echo "wo_site_project_name=$project_name" >> siteinfo.txt
            cd ..
        elif [[ "$site_type" =~ "php" || "$site_type" == "html" || "$site_type" == "alias" ]]; then
            echo "$color_blue[ INFO ] Taking backup of existing site for \"$site\"..."
            cp -a $site_path/htdocs .
            echo "$color_green[  OK  ] Backup of existing site taken successfully..."
            echo "$color_blue[ INFO ] Writing collected site information to siteinfo.txt..."
        ...
        tar -czf "$site-$(date +%A).tar.gz" "$site" && echo "$color_green[  OK  ] Daily backup compressed successfully..." || { echo "$color_red[ FAIL ] Failed to compress daily backup..."; return 1; }
        if [[ "$NO_UPLOAD" == "1" ]]; then
            cp "$site-$(date +%A).tar.gz" /root/ && echo "$color_yellow[ INFO ] Defined NO_UPLOAD variable, backup copied to /root/" || { echo "$color_red[ FAIL ] Failed to copy daily backup to /root/..."; return 1; }
        else
            mc cp "./$site-$(date +%A).tar.gz" "$MINIO_PATH/$HOSTNAME/$site/" &>/dev/null && echo "$color_green[  OK  ] Daily backup uploaded to MinIO successfully..." || { echo "$color_red[ FAIL ] Failed to upload daily backup to MinIO..."; return 1; }
        fi
        ...
        if [[ $(date -d "yesterday" +%m) != $(date +%m) ]]; then
            ...
            if [[ "$NO_UPLOAD" != "1" ]]; then
                mc cp "./$site-$(date -I).tar.gz" "$MINIO_PATH/$HOSTNAME/$site/" &>/dev/null && echo "$color_green[  OK  ] Monthly backup uploaded to MinIO successfully..." || { echo "$color_red[ FAIL ] Failed to upload monthly backup to MinIO..."; return 1; }
            fi
        fi
...
backup_status() {
    ...
    for site in ${local_sites[@]}; do
        ...
        [[ -z $remote_status ]] && { echo -e "$color_yellow[ INFO ] No backup found for \"$site\" on MinIO..."; continue; }
...
check_config() {
	[[ ! -e $@ ]] && { echo -e "$color_red[ FAIL ] File \"$@\" not found, aborting..."; exit 1; }
	. "$@"

	local vars=(ALARM_WEBHOOK_URL MINIO_PATH UPTIME_KUMA_API_URL)
	for var in "${vars[@]}"; do
		[[ -z ${!var} ]] && { echo -e "$color_red[ FAIL ] Variable \"$var\" not defined, aborting..."; return 1; }
	done
	return 0
}
...
#~ check crontab
check_crontab() {
	[[ "$(id -u)" != "0" ]] && { echo -e "$color_red[ FAIL ] This script must be run as root, aborting..."; return 1; }
...
check_database() {
    ...
    if [[ $? -ne 0 ]]; then
        ...
        mysql -e "CREATE DATABASE IF NOT EXISTS wordops;" &>/dev/null && echo "$color_green[  OK  ] Fact database created successfully..." || { echo "$color_red[ FAIL ] Failed to create fact database..."; return 1; }
        
        echo "$color_blue[ INFO ] Creating user..."
        mysql -e "CREATE USER 'wordops'@'localhost' IDENTIFIED BY 'wordops';" &>/dev/null && echo "$color_green[  OK  ] User created successfully..." || { echo "$color_red[ FAIL ] Failed to create user..."; return 1; }
...
database_manager() {
    ...
    "update")
        ...
        ;;
    "delete")
        mysql wordops -u wordops -pwordops -e "DELETE FROM wordops_facts WHERE web_url=$2;" &>/dev/null && echo -e "$color_green[  OK  ] Deleted successfully for $2..." || { echo -e "$color_red[ FAIL ] Failed to delete for $2..."; return 1; }
        ;;
    esac
    mysql wordops -u wordops -pwordops -e "UPDATE wordops_stats SET last_update=CURRENT_TIMESTAMP WHERE server_name='"$HOSTNAME"';" &>/dev/null && echo -e "$color_green[  OK  ] Updated successfully for $HOSTNAME..." || { echo -e "$color_red[ FAIL ] Failed to update for $HOSTNAME..."; return 1; }
...
download_backup() {
    ...
    printf "> "
    read site_name
    [[ -z $site_name ]] && { echo -e "$color_red[ FAIL ] Invalid site name, aborting..."; return 1; }
    printf "\n"
    ...
    [[ -z $backup_file_name ]] && { echo -e "$color_red[ FAIL ] Invalid backup file name, aborting..."; return 1; }
...
list_backups() {
    ...
    [[ "$list_mode" == "raw" ]] && { echo "${inventory[@]}" | sed 's/ /\n/g'; return 0; }
...
save_fact() {
    check_database
    if [[ "$1" == "all" ]]; then
        local site_facts=$(get_facts)
    else 
        local site_facts=$(get_facts $1)
    fi

    local site_facts_count=$(echo $site_facts | jq '. | length')
    for ((i = 0; i < $site_facts_count; i++)); do
        local web_url=$(echo $site_facts | jq -r ".[$i].web_url")
        local site_type=$(echo $site_facts | jq -r ".[$i].site_type")
        local php_version=$(echo $site_facts | jq -r ".[$i].php_version")
...
restore_backup() {
    ...
    [[ ! -e $backup_file ]] && { echo -e "$color_red[ FAIL ] Backup file not found, aborting..."; return 1; }
    
    cp -r $backup_file $tmpdir/

    cd $tmpdir || { echo -e "$color_red[ FAIL ] Failed to change directory to $tmpdir, aborting..."; return 1; }
    tar -xzf $backup_file && { echo -e "$color_green[  OK  ] Backup file extracted successfully..."; } || { echo -e "$color_red[ FAIL ] Failed to extract backup file, aborting..."; return 1; }
    . */siteinfo.txt

    [[ -z "$wo_site_name" ]] && { echo -e "$color_red[ FAIL ] Invalid backup file, aborting..."; return 1; }
    ...
    if [[ "$wo_site_type" == "wp" ]]; then
        ...
        wp --allow-root db export /var/www/$wo_site_name/backup/$wo_site_name.sql &>/dev/null && echo -e "$color_green[  OK  ] Database backup exported successfully in /var/www/$wo_site_name/backup/$wo_site_name.sql..." || { echo -e "$color_red[ FAIL ] Failed to export database backup..."; return 1; }
        popd &>/dev/null
        mv "${wp_config_file[1]}" "/var/www/$wo_site_name/backup/" || { echo -e "$color_red[ FAIL ] Failed to move wp-config.php to /var/www/$wo_site_name/backup/"; return 1; }
        mv "${wp_content_dir[1]}" "/var/www/$wo_site_name/backup/" || { echo -e "$color_red[ FAIL ] Failed to move wp-content to /var/www/$wo_site_name/backup/"; return 1; }
        ...
        mv "${wp_config_file[0]}" "${wp_config_file[1]}" || { echo -e "$color_red[ FAIL ] Failed to move wp-config.php to /var/www/$wo_site_name/htdocs/"; return 1; }
        mv "${wp_content_dir[0]}" "${wp_content_dir[1]}" || { echo -e "$color_red[ FAIL ] Failed to restore wp-content to /var/www/$wo_site_name/htdocs/"; return 1; }
        mv "$wo_site_db_file" "/var/www/$wo_site_name/htdocs/" || { echo -e "$color_red[ FAIL ] Failed to move sql database to /var/www/$wo_site_name/htdocs/"; return 1; }
        ...
        pushd /var/www/$wo_site_name/htdocs &>/dev/null
        wp --allow-root db import $wo_site_name.sql &>/dev/null && echo -e "$color_green[  OK  ] Database imported successfully from /var/www/$wo_site_name/htdocs/$wo_site_name.sql" || { echo -e "$color_red[ FAIL ] Failed to import database..."; return 1; }
        ...
        echo -e "$color_blue[ INFO ] Taking backup of existing site..."
        mv /var/www/$wo_site_name/htdocs /var/www/$wo_site_name/backup/ || { echo -e "$color_red[ FAIL ] Failed to move existing site files to /var/www/$wo_site_name/backup/"; return 1; }
        mysqldump -h localhost -u $db_user -p$db_pass $db_name > /var/www/$wo_site_name/backup/$wo_site_name.sql || { echo -e "$color_red[ FAIL ] Failed to take backup of existing database..."; return 1; }
        ...
        mv ${wo_site_root} /var/www/$wo_site_name/ || { echo -e "$color_red[ FAIL ] Failed to move site files to /var/www/$wo_site_name/"; return 1; }
        mysql -h localhost -u $db_user -p$db_pass $db_name < ${wo_site_db_file} || { echo -e "$color_red[ FAIL ] Failed to restore database..."; return 1; }
        echo -e "$color_green[  OK  ] Site files restored..."

        chown -R www-data: /var/www/$wo_site_name/htdocs
    elif [[ "$wo_site_type" =~ "php" || "$wo_site_type" == "html" ]]; then
        echo -e "$color_blue[ INFO ] Taking backup of existing site..."
        mv /var/www/$wo_site_name/htdocs /var/www/$wo_site_name/backup/ && echo -e "$color_green[  OK  ] Backup of existing site taken successfully in /var/www/$wo_site_name/backup/" || { echo -e "$color_red[ FAIL ] Failed to move existing site files to /var/www/$wo_site_name/backup/"; return 1; }
        mv ${wo_site_root} /var/www/$wo_site_name/ || { echo -e "$color_red[ FAIL ] Failed to move site files to /var/www/$wo_site_name/"; return 1; }
        echo -e "$color_green[  OK  ] Site files restored..."        
    ...
    wo stack restart &>/dev/null && echo -e "$color_green[  OK  ] WordOps backend restarted successfully..." || { echo -e "$color_red[ FAIL ] Failed to restart WordOps backend..."; return 1; }
